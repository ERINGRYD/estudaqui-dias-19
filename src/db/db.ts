...
import { dbReady, getDBOrThrow } from './singleton';
...
// Schedule save function - uses the database from context via scheduleSave hook
const scheduleSaveDB = async (): Promise<void> => {
  // This will be handled by the DBProvider context scheduleSave function
  // We keep this for backwards compatibility but it's essentially a no-op
  console.log('scheduleSaveDB called - handled by DBProvider context');
};
...
export const saveStudyPlan = (plan: StudyPlan): string => {
  ...
  try {
    ...
    // Save is handled by DBProvider context
    return planId;
  } catch (error) {
...
export const importDatabase = async (file: File): Promise<boolean> => {
  console.warn('importDatabase function is deprecated. Import functionality should be handled by DBProvider context.');
  return false;
};
